...
// Import your actual Poll type
import type { Poll } from '@/interface';

// Poll adapter function
...
export default function PollDetail() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status === 404) {
            ...
            setError(`Failed to load poll (Error ${response.status}). Please try again.`);
          }
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-gray-900 dark:to-blue-900/20 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Loading Poll</h2>
          <p className="text-gray-600 dark:text-gray-400">Fetching poll data...</p>
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-gray-900 dark:to-blue-900/20 flex items-center justify-center p-6">
          ...
          <CardContent className="space-y-4">
            <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                <strong>Requested Poll ID:</strong><br />
                <code className="bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded">{pollId}</code>
              </p>
            </div>
            ...
            <div className="text-xs text-gray-500 mt-4">
              <p>Try these test polls:</p>
              <div className="flex flex-wrap gap-2 justify-center mt-2">
                <Link href="/poll/test-poll-1" className="text-blue-600 hover:underline">test-poll-1</Link>
                <Link href="/poll/test-poll-2" className="text-blue-600 hover:underline">test-poll-2</Link>
                <Link href="/poll/test-poll-3" className="text-blue-600 hover:underline">test-poll-3</Link>
              </div>
            </div>
          </CardContent>
        </Card>
  ...
  if (!currentPoll) {
    return null; // This shouldn't happen due to error state above
  }
  ...
  const leadingOptions = currentPoll.options.filter(opt => opt.votes === leadingVotes && opt.votes > 0);
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {currentPoll.isActive ? (
                      ...
                      <Badge variant="warning" className="text-sm">Closed</Badge>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
                    <div className="flex items-center">
                      ...
                      <span className="font-semibold">{currentPoll.totalVotes}</span> votes
                    </div>
                    <div className="flex items-center">
                      <Eye className="h-4 w-4 mr-2" />
                      <span className="font-semibold">{currentPoll.views}</span> views
                    </div>
                    <div className="flex items-center">
                      <Clock className="h-4 w-4 mr-2" />
                      Created {new Date(currentPoll.createdAt).toLocaleDateString()}
                    </div>
                    {currentPoll.createdBy && currentPoll.createdBy !== 'unknown-user' && (
                      <div className="flex items-center">
              ...
              <CardContent className="pt-6">
                {!hasOptions ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">No voting options available.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {currentPoll.options.map((option, index) => {
                      const percentage = calculatePercentage(option.votes, currentPoll.totalVotes);
                      const isLeading = option.votes === leadingVotes && leadingVotes > 0;
                      ...
                      return (
                        <div
                          ...
                          } ${isLeading ? 'ring-2 ring-indigo-200 dark:ring-indigo-400' : ''}`}
                          onClick={() => !hasVoted && !voteLoading && handleVote(option.id)}
                        >
                          ...
                          {!hasVoted && voteLoading === option.id && (
                            <div className="flex justify-center mt-2">
                              ...
                              <span className="ml-2 text-sm text-gray-600">Recording vote...</span>
                            </div>
                          )}
                        </div>
                      );
            ...
            <Card className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-0 shadow-xl">
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-gray-900 dark:text-white">{currentPoll.totalVotes}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Options</span>
                  <span className="font-bold text-gray-900 dark:text-white">{currentPoll.options.length}</span>
                </div>
                <div className="flex justify-between items-center">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-gray-400">Your Vote</span>
